...
bool ImagePreProcessing::loadImage(const QString &filePath)
{
    ...
    qDebug()
        << "ImagePreProcessing: The path to the file has been obtained! Path to image: "
        << m_filePath;
...
bool ImagePreProcessing::save()
{
    ...
    if (cv::imwrite(fullPath.toStdString(), toSave))
    {
        ...
        qDebug()
            << "ImagePreProcessing: The image is saved in the application files: "
            << fullPath;
        return true;
    } else {
        qCritical()
            << "ImagePreProcessing: Error writing to: "
            << fullPath;
        return false;
    }
}
