...
void FileHandler::startPreprocessing()
{
    if (!currentImagePath().isEmpty())
    {
        ...
        qDebug()
            << "FileHandler: The image was transferred to the interface via the path:"
            << currentImagePath();

    } else {
        qDebug()
            << "FileHandler: ERROR! The Start command was not sent to the interface. No file was selected. Path:"
            << currentImagePath();
    }
}

void FileHandler::startCamera()
{
    emit startCameraManager();
}
...
{
    qDebug()
        << "FileHandler: The path to the preprocessed file has been obtained. filePath: "
        << filePath;
